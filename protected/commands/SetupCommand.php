...
class SetupCommand extends CConsoleCommand
{
	...
	public function run($args=array())
	{
		...
		switch ($args[0])
		{
			...
			case "generatefirstuser":
                ...
                break;
            case "test":
                echo $this->getEncryptedPassword('306824269@qq.com', 'zjwgy2012', Yii::app()->params['encryptionKey']);
                break;
			default:
				$this->showCommands();
		}		
	...
	{
		$this->log("SetupCommand: A command line helper for automating installation of CMS.");
		$this->log("===============================================================");
		$this->log("Usage:");
		$this->log("    php protected/yiic.php setup [arg1] [arg2] [arg3] [...] [argn]\n");
		$this->log("Arguments:");
		$this->log("    generatehash		Generates a hash for CMS to use for user data");
	...
	private function generateFirstUser($username, $password)
	{
		...
		$connection->createCommand('INSERT INTO users (id, email, password, nickname, user_role, status, created, updated) VALUES (NULL, :email, :password,  "superadmin", 1, 1, NOW(), NOW())')
